...
class FilterURL:
    ...
    def __init__(
        ...
        self.bad_path_comp: Optional[Pattern] = re.compile(path_re_to_compile) if path_re_to_compile else None
    ...
    def remove_sensitive(self, url: str, censored: str = "[...]") -> str:
        ...
        # 3. Censor path using the provided regex
        if self.bad_path_comp:
            match = self.bad_path_comp.search(new_path)
            if match:
                if self.bad_path_comp.groupindex:
                    temp_path_list = list(new_path)
                    for group_name in sorted(self.bad_path_comp.groupindex.keys(), reverse=True):
        ...
        return parsed._replace(netloc=new_netloc, query=new_query, path=new_path).geturl()
